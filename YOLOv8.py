...
def process_videos(directory):
    ...
    for video_path in sorted_videos:
        ...
        while frame_count < total_frames:
            ...
            # 수정된 부분: extract_pose_from_yolo()에 frame 전달
            ...
            frame_time_str = frame_time.strftime('%Y-%m-%dT%H-%M-%S')
            frame_number_str = f"{frame_count:04d}"  # 4자리로 프레임 번호 표시

            if "Very High Risk" in caption:
                high_risk_image_path = os.path.join(high_risk_dir, f'high_risk_frame_{frame_time_str}_{frame_number_str}.jpg')
            ...
            # 결과 이미지 저장
            output_path = os.path.join(output_dir, f'frame_{frame_time_str}_{frame_number_str}.jpg')
